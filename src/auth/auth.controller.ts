...
export class AuthController {
  ...
  @ApiOperation({
    summary: 'Créer un compte et obtenir une clef API',
    description:
      'Route publique. Retourne une clef API unique à utiliser dans le header X-API-Key.',
  })
  @ApiResponse({
    status: 201,
  ...
  @ApiHeader({
    name: 'X-API-Key',
    required: true,
    description: "Clef API obtenue à l'inscription",
  })
  ...
  getMe(@Request() req: AuthenticatedRequest) {
    const user = req.user!;
    return this.authService.getMe(user.key);
  }

  @ApiSecurity('api-key')
  @ApiOperation({
    summary: 'Régénérer la clef API',
    description: "L'ancienne clef est immédiatement invalidée.",
  })
  @ApiResponse({
    status: 200,
    description: 'Nouvelle clef API — retourne { apiKey }',
  })
  @ApiResponse({ status: 401, description: 'Clef API absente' })
  @Post('regenerate-key')
  @HttpCode(200)
  regenerateKey(@Request() req: AuthenticatedRequest) {
    const user = req.user!;
    return this.authService.regenerateKey(user.key);
  }

  @ApiSecurity('api-key')
  @ApiOperation({
    summary: 'Supprimer son compte',
    description: 'Action irréversible. La clef API est immédiatement révoquée.',
  })
  ...
  deleteAccount(@Request() req: AuthenticatedRequest) {
    const user = req.user!;
    this.authService.deleteAccount(user.key);
  }
}
